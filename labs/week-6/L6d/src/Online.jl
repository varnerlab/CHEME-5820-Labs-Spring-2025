...
function play(model::MyConstraintCheckingGameModel; 
    ϵ::Float64 = 0.01)

    # initialize - 
    T = model.T; # how many rounds do we play?
    η = model.η; # learning rate
    A = model.A; # constraint matrix
    b = model.b; # right hand side vector
    τ = model.τ; # summation of x upper bound
    ρ = model.ρ; # upper/lower of the elements of the matrix A
    weights = model.weights; # weights of the experts
    
    # size -
    n = size(A, 1); # number of constraints 
    m = size(A, 2); # number of variables (number of experts)
    one_vector = ones(Float64, n); # one vector

    # what is the initial solution -
    Φ = sum(weights[1, :]); # Φ is sum of the weights at time t
    x = τ*(weights[1, :]/Φ); # estimate of the solution now -
    ...
    while (should_continue == true)
        ...
        # compute the probability vector p -
        Φ = sum(weights[t, :]); # Φ is sum of the weights at time t
        x = τ*(weights[t, :]/Φ); # estimate of the solution now -

        # @show size(A), size(x), size(b), size(one_vector);
        
        # check the constraints -
        rhs = A*x - b - 2*ϵ*one_vector;
        found_a_solution = findall(x-> x ≤ 0, rhs) |> i-> length(i) == n ? true : false;

    
        if (found_a_solution == false)
            
            # ok, so we do NOT have a solution, we need to update the weights -
            violation = findall(x-> x > 0, rhs); # find the violation(s)
            for i ∈ violation
                for j ∈ 1:m
                    weights[t+1, j] = weights[t, j]*(1-η*A[i,j]); # update the weights
                end
            end
        end

        t += 1; # increment the time step
        if (t ≥ T || found_a_solution == true)
            should_continue = false; 
            
            # remove extra rows from w -
            weights = weights[1:(t-1),:];
        end
    end

    return x, found_a_solution, weights
end