...
function decode(tokens::Array{<:Number}, inversetokendict::Dict{Int64, String})::Array{String,1}

    # initialize -
    resultarray = Array{String,1}();
    ...
    for i ∈ tokens
        ...
        if (haskey(inversetokendict, i) == false)
            push!(resultarray, "<OOV>");
            continue;
        end
        push!(resultarray, inversetokendict[i]);
    end
